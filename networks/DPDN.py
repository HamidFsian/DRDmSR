...
class DPDN(nn.Module):
    def __init__(self,DPG_num_channels =1, DPG_base_channels = 24,DPG_num_residuals = 2,DDM_num_channels = 2, DDM_base_channels = 64, DDM_num_residuals =3):
        ...
        self.residual_layers = RG(DDM_base_channels, DDM_num_residuals)
        ...
        self.layers = torch.nn.Sequential(
                        nn.Conv2d(in_channels=1, out_channels=DDM_base_channels, kernel_size=9, stride=1, padding=4, bias=True),
                        nn.ReLU(inplace=True),
                        nn.Conv2d(in_channels=DDM_base_channels, out_channels=DDM_base_channels // 2, kernel_size=1, bias=True),
                        nn.ReLU(inplace=True),
                        nn.Conv2d(in_channels=DDM_base_channels // 2, out_channels=4, kernel_size=3, stride=1, padding=1, bias=True),
                        nn.PixelShuffle(2)
)  # Modified SRCNN
    ...
    def forward(self, mosaicked, sparse_image ): #, PPI_estimated
        #   filter pour 3by3       
        conv_filter_M = torch.Tensor(
            [[[   [1, 2, 2, 2, 1], [2, 4, 4, 4, 2], [2, 4, 4, 4, 2], [2, 4, 4, 4, 2], [1, 2, 2, 2, 1] ]]]).cuda() #M for 3by3 matrix [1 , 2 , 2 , 1], [2, 4, 4, 2], [2, 4, 4, 2], [1,  2, 2, 1]
        conv_filter_M = conv_filter_M.float()
        ...
        mosaicked = mosaicked.float()
        Im_raw = F.conv2d(mosaicked, conv_filter_M, padding='same') /  64.0   #  # 9*4 64.0 36    
        ...
        # Demosaic = F.conv2d(sparse_image, conv_filter_H, padding='same') / 1.0 # I_{WB}^k interpolation (pre demosaic)
        Demosaic = F.interpolate(sparse_image, scale_factor=1, mode='bicubic', align_corners=True)


        Demosaic = Demosaic.float()
        sparse_image = sparse_image.float()

        
        Demosaic_feature = self.extract2(torch.cat((Demosaic,sparse_image),1)) #extract2 => bloc1
        PPI_feature = self.extract(PPI_estimated) #extract => bloc2
        ...
        # output = self.layers(demosaic_estimated)  # Apply self.layers to demosaic_estimated
    #
        return PPI_estimated, demosaic_estimated#output
...
if __name__=='__main__':
    ...
    mosaicked = torch.tensor(mosaicked).cuda()
    PPI = torch.tensor(PPI).cuda()
    net = DPDN().cuda()
    outputs = net(mosaicked,PPI)
    outputs_array = tuple(tensor.detach().cpu().numpy() for tensor in outputs)
    outputs_array = np.array(outputs_array)
    print(outputs_array.shape) # (2, 32, 1, 60, 60) because one PPI and one demosaicked
