...
parser.add_argument("--model", default="checkpoint/pre-training/PPI_model_epoch_2000.pth", type=str, help="model path")
parser.add_argument("--dataset", default="./test_data", type=str, help="dataset name, Default: CAVE") #C:/Users/hamid/Desktop/python_projects/My_joint_DMSR/test_data/test_ARAD
...
with torch.no_grad():
    for ijk in range(knum):
        for image_name in sorted(os.listdir(image_list)):
            ...
            # target_demosaic = np.transpose(target_demosaic , (1,2,0))
            ...
            im = Image.open(opt.dataset + "/" + image_name)
            raw = np.array(im)
