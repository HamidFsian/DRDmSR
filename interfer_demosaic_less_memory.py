...
def mask_input(GT_image, msfa_size):
    # print("heda how shape te3 GT_image")
...
parser.add_argument("--model", default="checkpoint\main\main_model_epoch_3200.pth", type=str, help="model path")
parser.add_argument("--model_PPI", default="checkpoint/pre-training/PPI_model_epoch_2000.pth", type=str, help="model path")

#parser.add_argument("--model", default="checkpoint/fine-tuning/final_model_epoch_4000.pth", type=str, help="model path")
parser.add_argument("--dataset", default="C:/Users/hamid/Desktop/python_projects/My_joint_DMSR/test_data/tset_ARAD", type=str, help="dataset name, Default: CAVE")
...
PPI_net= DPG()
m_state_dict_PPI = torch.load(opt.model_PPI)
PPI_net.load_state_dict(m_state_dict_PPI['model'].state_dict())
    ...
    for ijk in range(knum):
        for i in range(1):
            for image_name in sorted(os.listdir(image_list)):
                ...
                im_gt_y = np.transpose(im_gt_y , (1,2,0))
                ###\u539f\u672c\u7684im_gt_y\u6309\u7167\u5b9e\u9645\u76f8\u673a\u6ee4\u6ce2\u9635\u5217\u6392\u5217
                im_l_y = mask_input(im_gt_y,opt.scale)
                print(im_l_y.shape)
                ...
                target_PPI = im_gt_y.sum(axis=0)/16.0
                ...
                for index in range(opt.scale**2):
                    ...
                    if cuda:
                        ...
                        model = model.cpu()
#  
                    # estimated_PPI = PPI_net(raw),estimated_PPI
                ...
                np.save("estimated_demosaic.npy",estimated_Demosaic)
                target_PPI = im_gt_y.sum(axis=0) / float(opt.scale**2)
                target_PPI = Variable(torch.from_numpy(target_PPI).float()).view(1, -1, target_PPI.shape[0], target_PPI.shape[1])
